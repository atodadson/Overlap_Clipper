...
Output fields per row:
  ...
  - Overlap Area (in layer CRS unitsÃÂ²)
...
class OverlapTableAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        return (
            "Scans a polygon layer for overlapping feature pairs "
            "and produces a table describing each overlap.\n\n"
            "For every pair (A, B) whose "
            "geometries intersect (share interior area), "
            "the algorithm records:\n"
            "  Ã¢â‚¬Â¢ The IDs of both features\n"
            "  Ã¢â‚¬Â¢ Any attribute fields you select "
            "(duplicated for both A and B, "
            "prefixed with 'a_' and 'b_')\n"
            "  Ã¢â‚¬Â¢ The overlap area (in CRS unitsÃÂ²)\n"
            "  Ã¢â‚¬Â¢ The overlap area as a percentage "
            "of Feature A's area\n"
            "  Ã¢â‚¬Â¢ The overlap area as a percentage "
            "of Feature B's area\n\n"
            "A QgsSpatialIndex is built once "
            "and used to narrow candidates before "
            "any precise geometry call, keeping the algorithm "
            "fast even on large layers."
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        _fields_param = self.FIELDS
        selected_field_names = self.parameterAsFields(
            parameters, _fields_param, context
        )
        ...
        if sink is None:
            raise QgsProcessingException(
                self.invalidSinkError(parameters, self.OUTPUT)  # noqa: E501
            )

        # ── 4. Load all features into memory & build spatial index ─────
        feedback.setProgressText(
            self.tr("Loading features and building spatial indexÃ¢â‚¬Â¦")
        )
        ...
        if total_features < 2:
            feedback.pushWarning(
                self.tr(
                    "Fewer than 2 valid features found — nothing to compare."
                )  # noqa: E501
            )
            return {self.OUTPUT: dest_id}
        ...
        feedback.setProgressText(self.tr("Detecting overlapsÃ¢â‚¬Â¦"))
        ...
        for i, fid_a in enumerate(fid_list):
            ...
            for fid_b in candidates:
                ...
                try:
                    ...
                    feedback.pushWarning(
                        self.tr(
                            f"Geometry error for pair ({fid_a}, {fid_b}): {e}"
                        )  # noqa: E501
                    )
                    continue
                ...
                out_feat.setAttribute(
                    "overlap_pct_a",
                    round(pct_a, 4) if pct_a is not None else None,
                )
                out_feat.setAttribute(
                    "overlap_pct_b",
        ...
        feedback.pushInfo(
            self.tr(
                f"Done. {overlap_count} overlapping pair(s) found "
                f"across {total_features} features."
            )
        )
